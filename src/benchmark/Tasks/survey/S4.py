...
def S4():
    ...
    act_open_dw = Act(dw_var, 'Open', "Dishwasher")
    goto_k.add_stmnt(scan_dws)
    goto_k.add_stmnt(let1)
    goto_k.add_stmnt(act_open_dw)
    ...
    act_open_drawer = Act(drawer_var, 'Open', 'Drawer')
    goto_k.add_stmnt(scan_drawers)
    goto_k.add_stmnt(let1)
    goto_k.add_stmnt(act_open_drawer)
    ...
    #Close Drawer and Dishwasher
    act_close_dw = Act(dw_var, 'Close', 'Dishwasher')
    act_close_drawer = Act(drawer_var, 'Close', 'Drawer')

    goto_k.add_stmnt(act_close_dw)
    goto_k.add_stmnt(act_close_drawer)
