...
def infeasible(prog, demo, env, exact=True):
    """
        ...
        for i in range(len(prog_ops)):
            if prog_ops[i] == 'BROKEN':
                ...
                if not found and not not_mandatory:
                    ...
                    return True
            #TEMP#elif len(demo_ops) <= i - num_non_acts_seen: #Too many statements generated
            elif len(demo_ops) <= i - num_non_acts_seen: #Too many statements generated
            #TEMP#    return True
                return True
            elif not prog_ops[i].partial_eq(demo_ops[i-num_non_acts_seen]):
                #Check if action is in rest of actions
                found = False
                #TEMP#for j in range(len(demo_ops)-i+num_non_acts_seen):
                for j in range(len(demo_ops)-i+num_non_acts_seen):
                #for j in range(len(demo_ops)):
                    #TEMP#if prog_ops[i].partial_eq(demo_ops[i+j-num_non_acts_seen]):
                    if prog_ops[i].partial_eq(demo_ops[i+j-num_non_acts_seen]):
